This section describes how to manage Spark applications in a Kubernetes cluster submitted to the 
...
For more information on how to prepare a CR file of the Spark application, refer to the [User Guide - Spark Application on Kubernetes](./user-guide.md#spark-application-on-kubernetes).

The possible ways to submit application CR files to a Kubernetes cluster are described below.
...
1. Edit the **spark-pi.yaml** file by setting the correct values for the following:
    * `metadata.namespace` - The namespace where the applications can be deployed, as described in the [Installation Guide - Prerequisites](/docs/installation-guide.md#prerequisites) section.
...
![alt text](/docs/public/images/airflow_spark_streaming_dag_graph_view.png "Airflow DAG to submit Spark apps")
...
* In order to monitor the submitted Spark application, [airflow.providers.cncf.kubernetes.sensors.spark_kubernetes](https://airflow.apache.org/docs/apache-airflow-providers-cncf-kubernetes/stable/_api/airflow/providers/cncf/kubernetes/sensors/spark_kubernetes/index.html) can be used.  
  ...
  ![alt text](/docs/public/images/airflow_spark_streaming_dag_sensor_task.png "Airflow DAG Sensor Task")
...
The following `securityContext` should be added in application CR to driver's and executor's configuration 
to support deployment on Kubernetes with Pod Security Standards Restricted profile enabled.
...
If executors fail, the driver creates new ones. If the driver pod fails, the Spark Operator initiates a new driver creation if only an appropriate restart policy is configured for the application. For the restart policy details, refer to [Spark Operator - Configuring Automatic Application Restart and Failure Handling documentation](https://github.com/GoogleCloudPlatform/spark-on-k8s-operator/blob/master/docs/user-guide.md#configuring-automatic-application-restart-and-failure-handling).
...
For more information, refer to the official Spark documentation at [https://spark.apache.org/docs/3.5.3/streaming-programming-guide.html#checkpointing](https://spark.apache.org/docs/3.5.3/streaming-programming-guide.html#checkpointing).
...
* S3  
   Refer to [S3 Storage](#s3-storage) for configuration details.
...
Refer to [spark-streaming-checkpoint sample app](../../spark-sample-apps/spark-streaming-checkpoint) for application examples.
...
The Operator itself can be deployed in the HA mode. For details, refer to [Installation Guide - High Available Deployment](/docs/installation-guide.md#high-available-deployment)
...
Spark applications can use S3 storage to store event logs, checkpointing, to read files, and to output files.
...
These jars are already included in Qubership Spark Images release.
